...
# (`angr[angrDB]==9.3.4`), so it moves with ../angr/python.nix: nixpkgs'
...
(angr-management.override { python312 = python; }).overridePythonAttrs (_: {
  ...
  src = fetchFromGitHub {
    ...
    tag = "v${version}";
    hash = "sha256-vsfdIRVZownOGHHffBOQhn7Nxn4XK2ii0/DMzBAK5qI=";
  };

  # angr: pinned `==9.3.4` and satisfied exactly, but the check compares against
